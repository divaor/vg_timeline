...
$(document).ready(function() {
  ...
  $.each($("#year_nav_list li"), function(key, year) {
    yearsListWidth += year.width();
  });
  // Holds the width of the years navigation container
  var yearsNavContWidth = $('#year_nav_slider').width();

  leftEdge = 0;
  rightEdge = yearsNavContWidth;
  // Observe when sliding right
  $(".year_text .previous_year").click(function(click) {
    slideContainer("right", yearsListWidth, click);
  });
  // Observe when sliding left
  $(".year_text .next_year").click(function(click) {
