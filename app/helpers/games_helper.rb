module GamesHelper
  ...
  def box_logo(game, size = :thumb, options = { :prototip => false, :tooltip => false, :hover => false })
    ...
    return image_tag(game.boxart.url(size), :alt => game.main_title, :name => game.full_title_colon_limit+"("+game.platform.short_name.upcase+")", :class => "boxart #{size.to_s} #{h}", :onmouseover => "tooltipGameInfo(this);") if (options[:prototip] and not options[:tooltip])
